...
async def 참가(ctx, handle):
    ...
    if handle not in entryDict.keys(): #핸들명이 참가자 목록에 존재하는지 확인
        try:
            #티어 정보 불러와서 저장
            if save_info_from_solvedac(handle): #참가자 정보를 크롤링하여 로컬 DB에 저장
                ctx.send("크롤링 성공")
            with open(f"userdata/profile_{handle}.json", 'r', encoding='utf-8') as userdata:
